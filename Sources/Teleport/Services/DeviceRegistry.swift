...
struct DeviceRegistry: Sendable {
    ...
    func service(for kind: DeviceKind) -> (any LocationSimulationService)? {
        let service = services.first(where: { $0.supportedKinds.contains(kind) })
    ...
    func shutdownAll() async {
        ...
        for service in services {
            let kinds = service.supportedKinds.map(\.rawValue).joined(separator: ",")
            do {
                try await service.clearLocation()
            } catch {
                TeleportLog.simulation.warning(
                    "Failed to clear location while shutting down \(kinds, privacy: .public): \(error.localizedDescription, privacy: .public)"
                )
            }
            await service.disconnect()
            TeleportLog.devices.debug("Disconnected service for \(kinds, privacy: .public)")
        }
    }
}
